...
const AddQuestion = () => {
    ...
    const send_question_data = () => {
        ...
        axios
            ...
            .then(function (resp) {
                if (resp.status === 201) {
                    setshow(false);
                    toast.success(`${toTitleCase(question)}  Added successfully !`, { position: "top-center", autoClose: 2000, });
                    navigate(-1);
                }
            })
            .catch((err) => {
    ...
    const update_question_data = () => {
        ...
        axios
            .put(
                ServerAddress + "ems/put_invite_user/" + question_id,
                {
                    question: question ? toTitleCase(question).toUpperCase() : question,
                    option1: option1 ? toTitleCase(option1).toUpperCase() : option1,
                    option2: option2 ? toTitleCase(option2).toUpperCase() : option2,
            ...
            .then(function (resp) {
                ...
                if (resp.status === 200) {
                    setshow(false);
                    toast.success(`${toTitleCase(question)}   Updated successfully !`, { position: "top-center", autoClose: 2000, });
                    navigate(-1);
                }
            })
            .catch((err) => {
                                ...
                                <Row>
                                    ...
                                    <Col lg={3} md={6} sm={6}>
                                        <div>
                                            ...
                                            <NSearchInput
                                                data_list={answare_list}
                                                data_item_s={answare}
                                                set_data_item_s={setansware}
                                                set_id={setansware_id}
                                                show_search={false}
                                                show_error={true}
                                                error_s={answare_err}
                                                error_message={"Answare is required"}
                                            />
                                        </div>
                                    </Col>
