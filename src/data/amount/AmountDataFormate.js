...
const AmountDataFormate = ({ data, data1, can_delete }) => {
  ...
  const renderCell = (text, maxLength = 60, showTooltip = true) => {
    ...
    return (
      <OverlayTrigger
        placement="top"
        overlay={<Tooltip>{toTitleCase(trimmed)}</Tooltip>}
      >
      ...
      {currentData?.length === 0 ? (
        ...
        currentData.map((quiz, idx) => (
          <tr key={quiz.id} style={{ borderBottom: "1px solid #eee" }}>
            ...
            <td>
              <Link to="/amount/add" state={{ data: quiz }}>
                {quiz.transactionId}
              </Link>
            </td>
...
export default AmountDataFormate;
