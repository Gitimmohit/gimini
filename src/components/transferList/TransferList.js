  ...
  return (
    <div className="d-flex flex-wrap">
              ...
              onClick={() => {
                if (disabled === false) {
                  if (filter_a.length > 0) {
                    ...
                    let temp_selected = list_a;

                    // Filter out duplicates from temp_selected
                    let filtered_temp_selected = temp_selected.filter(
                      (item) => {
                        // Convert each item to a string for comparison
                        let itemString = JSON.stringify(item);
                        // Check if the item is not present in list_b
                        return !list_b.some(
                          (existingItem) =>
                            JSON.stringify(existingItem) === itemString,
                        );
                      },
                    );
                    setlist_b([...list_b, ...filtered_temp_selected]);
                    setlist_a([]);
                    setselected_a([]);
                  }
                }
            ...
            <BiChevronRight
              ...
              onClick={() => {
                if (disabled === false) {
                  ...
                  // Filter out duplicates from temp_selected
                  let filtered_temp_selected = temp_selected.filter((item) => {
                    // Convert each item to a string for comparison
                    let itemString = JSON.stringify(item);
                    // Check if the item is not present in list_b
                    return !list_b.some(
      ...
      <Col lg={width ? 5 : 4} bd={12} sm={12} xs={12}>
        ...
        <div className="card scrollBarWidth" style={{ overflow: "auto", height: "200px" }}>
          <div
            className="card-body p-2"
            style={{ border: "1px solid #D3D3D3" }}
          >
